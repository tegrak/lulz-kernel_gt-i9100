...
#define ARMCLOCK_1204MHZ		1204000
...
/* Using lookup table to support 1204MHz/1000MHz by reading chip id */
		...
		unsigned int relation)
{
	...
	if (ondemand || conservative || lulzactive || interactive) {
		check_gov = 1;
		if (relation & ENABLE_FURTHER_CPUFREQ)
			...
			enum cpufreq_level_request cpufreq_level)
{
	...
	/* touch screen lock the minimum freq at 500MHz by tegrak */
	if (s5pv310_max_armclk == ARMCLOCK_1204MHZ) {
		if (cpufreq_level != CPU_L0) {
			cpufreq_level += 2;
		}
		...
		unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
		/* changed to L3 @1.0GHz by tegrak */
		ret = cpufreq_driver_target(cpufreq_cpu_get(cpu),
...
static int s5pv310_update_dvfs_table(void)
{
	...
	case 1204000:
		printk(KERN_INFO "armclk set max 1204MHz as default@@@@@\n");
		break;
